...
export function WagerGrid({
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-5">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="bg-card border border-border dark:border-border/80 rounded-xl p-4 lg:p-5 shadow-sm">
  ...
  if (wagers.length === 0) {
    return (
      <div className="text-center py-16 lg:py-20 bg-card border border-border dark:border-border/80 rounded-xl lg:rounded-2xl shadow-sm">
        <div className="max-w-md mx-auto px-4">
          <div className="h-20 w-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-primary/10 to-primary/5 dark:from-primary/20 dark:to-primary/10 flex items-center justify-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            {(searchQuery || selectedCategory) && (
              <button
                ...
                className="inline-flex items-center justify-center gap-2 bg-muted dark:bg-muted/80 text-foreground px-6 py-3 rounded-lg font-medium hover:bg-muted/80 dark:hover:bg-muted/60 transition-all active:scale-95 touch-manipulation focus:outline-none focus:ring-2 focus:ring-primary/50 min-h-[44px]"
              >
                Clear Filters
              </button>
            )}
