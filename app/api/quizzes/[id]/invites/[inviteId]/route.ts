...
export async function DELETE(
  ...
  try {
    ...
    const response = await nestjsServerFetch<any>(`/quizzes/${id}/invites/${inviteId}`, {
      method: 'DELETE',
      token,
      requireAuth: true,
    });
    ...
    if (!response.success) {
      return appErrorToResponse(new Error(response.error?.message || 'Failed to revoke invitation'));
    }

    return successResponseNext(response.data || { message: 'Invitation revoked' });
  } catch (error) {
