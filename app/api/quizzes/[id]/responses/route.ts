...
export async function GET(
  ...
  try {
    ...
    // Call NestJS backend to get quiz responses (let backend handle auth validation)
    ...
    if (!response.success) {
      return appErrorToResponse(new Error(response.error?.message || 'Failed to fetch quiz responses'));
    }
