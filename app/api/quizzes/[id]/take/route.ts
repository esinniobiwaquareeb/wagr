...
export async function POST(
  ...
  try {
    ...
    // Call NestJS backend to take quiz (let backend handle auth validation)
    ...
    if (!response.success) {
      return appErrorToResponse(new Error(response.error?.message || 'Failed to process quiz action'));
    }
